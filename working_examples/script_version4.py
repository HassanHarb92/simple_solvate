...
def parse_input_file(filename):
    ...
    # Validate counter ion input
    if "--counter_ion" in params and params["--counter_ion"].upper() != "NONE":
        required_counter_fields = ["--counter_ion_count", "--counter_ion_SMILES"]
        for field in required_counter_fields:
            if field not in params:
                raise ValueError(f"[ERROR] Missing required counter ion field: {field}")

    # Validate solvent input
    if "--solvent" in params and params["--solvent"].upper() != "NONE":
        required_solvent_fields = ["--solvent_SMILES"]
        for field in required_solvent_fields:
            if field not in params:
                raise ValueError(f"[ERROR] Missing required solvent field: {field}")
...
def place_counter_ions(main_coords, counter_ion_xyz, count):
    ...
    max_distance = calculate_max_distance(center, main_coords) + 3.0  # Ensure spacing

    counter_ions = []
    for i in range(count):
        sign = 1 if i % 2 == 0 else -1  # Opposite placements
...
def run_xtb_optimization(xyz_file, charge, solvent):
    ...
    if result.returncode != 0:
        print("[ERROR] xTB optimization failed:", result.stderr)
    else:
...
def main():
    ...
    if params["--counter_ion"].upper() != "NONE":
        ...
        counter_ions = place_counter_ions(main_coords, counter_ion_xyz, int(params["--counter_ion_count"]))
        all_coords += counter_ions
    ...
    create_frozen_input(len(main_coords))
